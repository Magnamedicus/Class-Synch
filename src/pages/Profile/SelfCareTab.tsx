// src/pages/Profile/SelfCareTab.tsx
...
function buildDefaultAnswers(): AnswerMap {
    const out: AnswerMap = {};
    (QUESTIONS.selfCare.questions ?? []).forEach((q) => {
        if (q.defaultValue !== undefined) out[q.id] = q.defaultValue;
        else if (q.type === "priority") out[q.id] = 70; // same convention as QuestionnairePage
    });
    return out;
}
...
const SelfCareTab: React.FC = () => {
    ...
    // Renderer for each self-care question type
    const renderInput = (q: (typeof QUESTIONS)["selfCare"]["questions"][number]) => {
        ...
        switch (q.type) {
            ...
            case "selfcare-selector": {
                const v: SelfCarePrefs[] = Array.isArray(value) ? value : [];
                return (
                    <SelfCareSelector
                        value={v}
                        onChange={(nv) => updateAnswer(q.id, nv)}
    ...
    return (
        <div className="profile-tab-pane profile-tab--selfcare">
            ...
            <div className="profile-grid">
                {selfCareQs
                    .filter((q) => isVisible(q, answers)) // respect selfcare_boolean gating
                    .map((q) => (
